...
def _api_manifest(base_url: str, generated_at: str) -> dict[str, Any]:
    ...
    endpoints.append(
        {
            "path": "/api/translation-status.json",
            "description": "Per-locale translation coverage (percent of canonical English keys defined).",
            "format": "json",
        }
    )
    return {
...
def _publish_api_mirror(root: Path, api_dir: Path, *, brotli: bool) -> dict[str, int]:
    ...
    # Translation coverage (locales/ -> per-locale percentages). Generated by
    # the build itself — not only by the Node validator — so the published
    # document is reproducible and the mirror's pruning keeps it alive.
    status = _translation_status_document(root)
    if status is not None:
        target = api_dir / "translation-status.json"
        ...
        if _write_gzip(target) is not None:
            written.add(target.name + ".gz")
            gz_files += 1
        if brotli and _write_brotli(target) is not None:
            written.add(target.name + ".br")
            br_files += 1
...
# ---------------------------------------------------------------------------
# Translation coverage document (api/translation-status.json)
# ---------------------------------------------------------------------------


def _flatten_locale(mapping: dict[str, Any], prefix: str = "") -> dict[str, Any]:
    """Flatten nested locale mappings to ``dotted.key -> leaf`` (like the JS validator).

    Arrays and ``null`` are leaf values, matching ``scripts/validate-translations.js``.
    """
    flat: dict[str, Any] = {}
    for key, value in mapping.items():
        path = f"{prefix}.{key}" if prefix else key
        if isinstance(value, dict):
            flat.update(_flatten_locale(value, path))
        else:
            flat[path] = value
    return flat


def _translation_status_document(root: Path) -> dict[str, int] | None:
    """Compute per-locale translation coverage from ``locales/``.

    This is the same document ``scripts/validate-translations.js`` (the CI
    gate) writes, but generated by the offline build itself so the published
    ``api/translation-status.json`` is a reproducible artifact: the
    ``translation-status/`` page fetches it, and a file that only a Node
    script produces would be pruned by every pipeline run and 404 in
    production (and fail ``check_reproducible.py`` whenever it was
    committed). Coverage is the share of canonical English keys a locale
    defines, rounded the way ``Math.round`` does (half up) so both
    implementations stay byte-identical. Returns ``None`` when the repository
    carries no locales, in which case the document is not published.
    """
    ...
    canonical = _flatten_locale(load(en_path))
    total = len(canonical)
    if not total:
        return None

    report: dict[str, int] = {}
    # Sorted for a deterministic key order on every filesystem (the JS side
    # sorts its readdir list for the same reason).
    for locale_path in sorted(locales_dir.glob("*.json")):
        defined = _flatten_locale(load(locale_path))
        missing = sum(1 for key in canonical if key not in defined)
        # math.floor(x + 0.5) == Math.round(x) for the non-negative percentages here.
        report[locale_path.stem] = math.floor((total - missing) / total * 100 + 0.5)
    return report
